...
# First, let's add a Swish activation function
def swish(x):
...
# Build the CNN model with Swish activation and improved architecture
def build_cnn_model(input_shape):
    model = Sequential([
        # First convolutional block
        Conv2D(16, (3, 3), activation='swish', padding='same', input_shape=input_shape),
        BatchNormalization(),
        MaxPooling2D((2, 2)),
        Dropout(0.4),
        ...
        Flatten(),
        Dense(128, activation='swish', kernel_regularizer=tf.keras.regularizers.l2(0.0005)),
        BatchNormalization(),
        Dropout(0.1),
        Dense(64, activation='swish', kernel_regularizer=tf.keras.regularizers.l2(0.0005)),
        BatchNormalization(),
        Dropout(0.1),
        Dense(1, activation='sigmoid')
    ])
    
    # Compile the model with a custom optimizer configuration
...
# Add improved learning rate scheduler
...
# Set up callbacks with improved configuration
callbacks = [
...
log_message("Starting model training with Swish activation and optimized parameters...")
batch_size = 16  # Slightly larger batch size for more stable gradients
